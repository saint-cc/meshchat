...
- **Are mandatory-signed**, dropped outright on a missing or invalid signature — same tier as `app:migrate`/`app:burn`/`call:*`/`shell:*`, since these packets drive real cryptographic session state rather than just display.
...
See [Signal Server Protocol](#signal-server-protocol) for both types' table rows, and `X4DH.md` for the DH derivations, the propose-freshness/replay guards, and the stuck-at-RK0 detection and bounded automatic retry that keep a session from staying wedged at `RK0` indefinitely when a live upgrade attempt goes unanswered.
