...
const state = {
  ...
  keys: null, cryptoKey: null, encKey: null, signKey: null,
  contacts: {}, peerBackups: {}, pendingExchange: {},
  currentChat: null, ws: null, online: new Set(),
  unread: {}
};

const isSecure        = window.location.protocol === "https:";
const SIGNAL_URL = isSecure ? `wss://${window.location.host}/ws/` : `ws://172.16.3.35:8888`;
...
function setRandomLoginNotice() {
  const notices = [
    ...
    "<strong>hint —</strong> messages may arrive later if a device was offline.<br>this is normal in a distributed system.",
    ...
    "<strong>hint —</strong> your contacts live on your device and your network.<br>but back them up if you care about them.",
    ...
    "<strong>hint —</strong> you can always reconnect later.<br>messages don't require everyone to be online at once.",
    "<strong>hint —</strong> delays, duplicates or reordering can happen.<br>the system will sort it out.",
    "<strong>hint —</strong> if you run your own node, others can connect to you.<br>just like you connect to them.",
...
function serialiseContacts() {
  ...
  for (const [id,c] of Object.entries(state.contacts))
    out[id] = { name: c.name, publicId: c.publicId, shareableKey: c.shareableKey,
                messages: c.messages.slice(-5).map(m => m.type === "audio" ? {...m, data:null, expired:true} : m),
...
setInterval(() => saveContactsBackup(true), 10 * 60 * 1000);
...
async function pushBackupToContacts(blob) {
  for (const id of Object.keys(state.contacts)) {
    if (!state.online.has(id)) continue;
    ...
    sendSignal({ type: "backup_push", from: state.publicId, to: id, blob });
  }
}
...
async function sendRestoreRequest(id) {
  ...
  sendSignal({ type: "push_restore_request", from: state.publicId, to: id, blob });
  mlog.info(`→ RESTORE_REQ  to   ${pid(id)}`);
}

async function handleRestoreRequest(msg) {
  if (!msg.from || !msg.blob) return;
  try {
    const plain = await decryptObject(state.encKey, msg.blob);
  ...
  if (Object.keys(state.contacts).length <= 1) {
    mlog.info(`← RESTORE_REQ  from ${pid(msg.from)} — fresh client, requesting back`);
    sendRestoreRequest(msg.from);
  }

  mlog.info(`← RESTORE_REQ  from ${pid(msg.from)} — sending ack`);
  sendSignal({ type: "push_restore_ack", from: state.publicId, to: msg.from });
}
...
async function handleMsgExchange(msg) {
  ...
  if (!msg.reply) {
    ...
    state.pendingExchange[msg.from] = msg.msgs || [];
    ...
    contact.messages = mergeMessages(contact.messages, state.pendingExchange[msg.from]);
    mlog.debug(`SYNC merge +${contact.messages.length - before} msgs from ${pid(msg.from)}`);
    delete state.pendingExchange[msg.from];
  } else {
    const before = contact.messages.length;
...
function handleSignal(msg) {
  switch(msg.type) {
    ...
    case "online_list":
      mlog.debug(`SIG online_list [${(msg.ids||[]).map(pid).join(", ")}]`);
      (msg.ids || []).forEach(id => {
        if (id === state.publicId) {
          markOnline(id);
          if (sessionFresh) {
    ...
    case "push_restore_request": handleRestoreRequest(msg); break;
    case "push_restore_ack":     handleRestoreAck(msg);     break;
    case "restore_push":         handleRestorePush(msg);    break;
    case "message":              receiveMessage(msg);       break;
    case "msg_exchange":         handleMsgExchange(msg);    break;
    case "backup_push":          handleBackupPush(msg);     break;

    default: mlog.debug(`SIG unknown type=${msg.type}`);
  }
}

function sendSignal(obj) { if (state.ws?.readyState === WebSocket.OPEN) state.ws.send(JSON.stringify(obj)); }
...
async function sendAudioMessage(blob) {
  ...
  reader.onloadend = async () => {
    ...
    // encrypt for transit
    const payload   = { id, type: "audio", data: base64, mimeType, ts };
    ...
    sendSignal({ type: "message", from: state.publicId,
    ...
    await saveContacts();
    renderMessages();
    mlog.info(`→ AUDIO        to   ${pid(state.currentChat)}  size=${blob.size}b`);
  };
...
async function receiveMessage(msg) {
  ...
  try {
    ...
    const valid=msg.sig&&contact.signPublicKey?verifyBlob(msg.blob,msg.sig,contact.signPublicKey):false;
    ...
    if (plain.type === "audio") {
      ...
      msgObj.type    = "audio";
      msgObj.mimeType = plain.mimeType;
      mlog.info(`← AUDIO        from ${pid(msg.from)}  sig:${valid ? "✓" : "✗"}`);
    } else {
...
async function sendMessage() {
  ...
  const blob = await encryptMessage(contact.encKey, { id, text, ts });
  const sig  = await signBlob(blob);
  sendSignal({ type: "message", from: state.publicId, to: state.currentChat, blob, sig });
  contact.messages.push({ id, from: state.publicId, text, ts, valid: true });
  mlog.info(`→ MSG          to   ${pid(state.currentChat)}`);
...
async function addContact(name,shareableKey,save=true){
  ...
  mlog.info(`CONTACT    added ${name}  ${pid(publicId)}`);
  renderContactList();
  return true;
}
...
function updateOnlineStatus(id, on) {
  if (on) markOnline(id);
  else { state.online.delete(id); dotTimestamps[id] = null; }
}
...
function contactAction(action) {
  ...
  if(action==="edit"){
	  ...
	  btns.innerHTML='<button class="btn-cancel" onclick="closeContactAction()">CANCEL</button>'+'<button class="btn-confirm" id="contactActionConfirm">SAVE</button>';
	  ...
	  document.getElementById("contactActionConfirm").onclick=async()=>{
		...
		if(newKey){
		  try{
			...
			mlog.info(`CONTACT    key updated for ${c.name}  ${pid(c.publicId)}`);
		  }catch(e){
...
document.getElementById("loginButton").onclick = async (e) => {
  ...
  try {
	...
	state.signPublicKey=ed25519.getPublicKey(keys.signingKeySeed);
	state.shareableKey=rawToBase64(keys.encryptionKey)+"."+rawToBase64(state.signPublicKey);
	...
	if(!state.contacts[state.publicId]){
	  ...
	  const encKeyBytes=base64ToRaw(parts[0]);
	  state.contacts[state.publicId]={name:state.user+" (me)",publicId:state.publicId,shareableKey:state.shareableKey,encKey:await importEncKey(encKeyBytes),signPublicKey:state.signPublicKey,messages:[]};
	}else{
	  // patch existing me contact with new bundled key and signPublicKey
	  state.contacts[state.publicId].shareableKey=state.shareableKey;
	  state.contacts[state.publicId].signPublicKey=state.signPublicKey;
	}
    loadPeerBackups();
...
document.getElementById("modalConfirm").onclick = async () => {
  ...
  if (!name || !key) return;
  if (await addContact(name, key)) closeModal();
};
